...
export async function getServerSideProps() {
  ...
  return { props: { corpIDsJSON, employeeIDsJSON, managerIDsJSON, managerInit, employeeInit, corpIDInit } };
}
  ...
  return (
    <div className={styles.container}>
      ...
      <Grid container spacing={1}>
        ...
        <Grid item xs={2} />
        <Grid item xs={4}>
          <TextField required label="Bank ID" fullWidth value={bankID} onChange={(e) => setBankID(e.target.value)} />
        </Grid>
        <Grid item xs={4}>
        ...
        <Grid item xs={2} />
        <Grid item xs={4}>
          <TextField type="number" label="Reserved Assets" fullWidth value={resAssets} onChange={(e) => setResAssets(e.target.value)}/>
        </Grid>
        <Grid item xs={4}>
          <FormControl fullWidth>
            <InputLabel>Corporation ID</InputLabel>
            <Select label="Corporation ID" value={corpID} onChange={(e) => setCorpID(e.target.value)}>
              {props.corpIDsJSON.map((obj, i) => {
                let corpID = obj["corpID"];
                return <MenuItem key={i} value={corpID}>{corpID}</MenuItem>;
              })}
            </Select>
          </FormControl>
        </Grid>
          ...
          <FormControl fullWidth>
            ...
            <Select label="Employee" value={employee} onChange={(e) => setEmployee(e.target.value)}>
              {props.employeeIDsJSON.map((obj, i) => {
                let perID = obj["perID"];
                return <MenuItem key={i} value={perID}>{perID}</MenuItem>;
              })}
            </Select>
          </FormControl>
        </Grid>
